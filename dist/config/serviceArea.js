...
exports.isRouteServiceable = isRouteServiceable;
/**
 * Service area boundaries for the United Kingdom
 */
exports.UK_BOUNDARIES = {
...
exports.EXCLUDED_AREAS = [
    {
        ...
        message: "We don't currently service the remote Scottish Highlands. Please select a location further south.",
    },
    {
        name: "Outer Hebrides",
 ...
 */
function isLocationInUK(location) {
    return (location.lat >= exports.UK_BOUNDARIES.southwest.lat &&
        location.lat <= exports.UK_BOUNDARIES.northeast.lat &&
        location.lng >= exports.UK_BOUNDARIES.southwest.lng &&
        location.lng <= exports.UK_BOUNDARIES.northeast.lng);
}
/**
 ...
 */
function isLocationInExcludedArea(location) {
    // Check polygon-based exclusion zones
    for (const area of exports.EXCLUDED_AREAS) {
        if ("polygon" in area) {
            ...
            const distance = getDistanceFromLatLonInKm(location.lat, location.lng, area.center.lat, area.center.lng);
 ...
 */
function isLocationServiceable(location) {
    ...
    const isMainlandUK = location.lat >= exports.UK_BOUNDARIES.southwest.lat &&
        location.lat <= exports.UK_BOUNDARIES.northeast.lat &&
        location.lng >= exports.UK_BOUNDARIES.southwest.lng &&
        location.lng <= exports.UK_BOUNDARIES.northeast.lng;
    if (!isMainlandUK) {
        // Check if it's on a serviced island
        for (const island of exports.SERVICED_ISLANDS) {
            const distance = getDistanceFromLatLonInKm(location.lat, location.lng, island.center.lat, island.center.lng);
        ...
        return {
            serviceable: false,
            message: "We don't currently service this location. Please select a location on the UK mainland or a major island.",
        };
    }
 ...
 * @returns An object with the serviceable status and a message if not serviceable
 */
function isRouteServiceable(pickup, dropoff, distance_miles) {
    ...
    if (distance_miles !== undefined && distance_miles > exports.MAX_SERVICE_DISTANCE) {
        return {
            serviceable: false,
            message: `We don't currently support journeys longer than ${exports.MAX_SERVICE_DISTANCE} miles. Your journey is approximately ${Math.round(distance_miles)} miles.`,
        };
    }
    return { serviceable: true };
}
