...
class FareService {
    ...
    static async getRouteDetails(pickupLocation, dropoffLocation, additionalStops) {
        try {
            ...
            const coordinates = [
                ...
                .join(";");
            const response = await axios_1.default.get(`${this.BASE_URL}/${coordinates}`, {
                params: {
    ...
    static async calculateFare(request) {
        try {
            const { pickupLocation, dropoffLocation, additionalStops, vehicleType } = request;
            // Get base rate for vehicle type or use default
            const baseRate = this.VEHICLE_BASE_RATES[vehicleType] ||
                this.VEHICLE_BASE_RATES["Standard Saloon"];
            const minimumFare = this.MINIMUM_FARES[vehicleType] ||
                this.MINIMUM_FARES["Standard Saloon"];
            // Get route details from Mapbox
            const routeDetails = await this.getRouteDetails(pickupLocation, dropoffLocation, additionalStops);
            // Calculate fare
            // Base formula: base_rate + (distance_km * rate_per_km) + (duration_min * rate_per_min)
            const ratePerKm = 1.5; // £1.50 per kilometer
            const ratePerMin = 0.5; // £0.50 per minute
            let fareEstimate = baseRate +
                routeDetails.distance * ratePerKm +
            ...
            return {
                fareEstimate,
                distance_km: Math.round(routeDetails.distance * 10) / 10, // Round to 1 decimal place
